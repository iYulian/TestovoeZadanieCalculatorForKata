...
public class Calculator {
    public static String calc(String input) {
        ...
        try {
            ...
            byte [] informaciya = reshenie.informaciya(zadacha); // Собираем начальную информацию о строке введенной с клавиатуры

            if (informaciya[2] == 1) { // Путь если задание в римских числах
                zadacha = reshenie.perevodRimArab(zadacha, informaciya); // Переводим римское число в арабское
                String otvet = reshenie.raschet(zadacha, informaciya); // Считаем в действие арабских числах
                informaciya[2] = 0;
...
class  Reshenie {
    ...
    String perevodRimArab (String zadacha,byte [] rezultatProverki) throws ThrowException{
        ...
        if (rezultatProverki[2] == 1) {
            ...
            if (zadacha.contains("-")) throw new ThrowException("throws Exception //т.к. в римской системе нет отрицательных чисел");
